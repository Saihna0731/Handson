...
Alpine.data('categoryPage', ({ catId, categories }) => ({
	...
	async init() {
		try {
			...
			if (label && label !== 'Ангилалаар') {
				this.$store.shop.category = `cat:${label}`;
			}
		} catch (e) {
	...
	get filtered() {
		...
		const desired = `cat:${this.categoryLabel}`;
		...
		return this.books.filter((b) => {
			...
			const matchC = this.$store.shop.categoryId(b) === desired;
...
Alpine.data('bookDetailPage', ({ id }) => ({
	...
	get relatedBooks() {
		...
		if (!cat) return others.slice(0, 8);

		const same = others.filter((b) => String(b?.category ?? '').trim() === cat);
		if (same.length >= 8) return same.slice(0, 8);
